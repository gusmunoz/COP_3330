...
MyInt operator+(const MyInt& x, const MyInt& y){
    ...
    int remainder = 0, columnTotal = 0, colPlusRem = 0;
 
    for(int i = 0; i<s.size - 1; i++){          //iterate from end to begin of array
        cout << "sumTotal: " << columnTotal <<'\n';
 
        //if the summation is larger than 1 digit
        if((columnTotal = C2I(x.array[i])+C2I(y.array[i])) > 9){
            colPlusRem = columnTotal + remainder;       //store full value
            remainder = colPlusRem - 9;         //store remainder
            colPlusRem = colPlusRem - remainder;
                    //add single int to array representing sum
        }
        s.array[i] = colPlusRem;
        s.size = sumCounter;
    }
    cout << "sumTotal: " << endl;
